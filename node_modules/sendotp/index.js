...
class SendOtp {
    ...
    constructor(authKey, messageTemplate) {
        ...
        if(messageTemplate){
            ...
            this.messageTemplate = ["Your OTP is {{otp}}. Please do not share it with anybody.","You have an upcoming vaccination. Kindly report in time. MessageId: {{otp}}","You have a pending vaccination. Kindly report as soon as possible in your AWC. MessageId: {{otp}}"];
        }
    ...
    send(contactNumber, senderId, otp, callback) {
        ...
        let args = {
                ...
                sender: senderId,
                message: this.messageTemplate[0].replace('{{otp}}', otp),
                otp: otp,
                otp_expiry: this.otp_expiry
            };
        return SendOtp.doRequest('get', "sendotp.php", args, callback);
    }
    sendup(contactNumber, senderId, otp, callback) {
        if (typeof otp === 'function') {
            callback = otp;
            otp = SendOtp.generateOtp()
        }
        let args = {
                ...
                mobile: contactNumber,
                sender: senderId,
                message: this.messageTemplate[1].replace('{{otp}}', otp),
                otp: otp,
                otp_expiry: this.otp_expiry
            };
        return SendOtp.doRequest('get', "sendotp.php", args, callback);
    }
    sendpre(contactNumber, senderId, otp, callback) {
        if (typeof otp === 'function') {
            callback = otp;
            otp = SendOtp.generateOtp()
        }
        let args = {
                ...
                mobile: contactNumber,
                sender: senderId,
                message: this.messageTemplate[2].replace('{{otp}}', otp),
                otp: otp,
                otp_expiry: this.otp_expiry
            };
        return SendOtp.doRequest('get', "sendotp.php", args, callback);
